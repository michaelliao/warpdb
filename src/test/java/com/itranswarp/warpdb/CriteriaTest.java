...
public class CriteriaTest {
    ...
    public void testSelect() {
        ...
        assertEquals("SELECT * FROM user WHERE name = ?", warpdb.select().from(User.class).where("name = ?", "Bob").sql());
        assertEquals("SELECT * FROM user WHERE name = ? AND the_tag > ?", warpdb.select().from(User.class).where("name = ?", "Bob").and("tag > ?", 10).sql());
        assertEquals("SELECT * FROM user WHERE name = ? ORDER BY name", warpdb.select().from(User.class).where("name = ?", "Bob").orderBy("name").sql());
        ...
        assertEquals("SELECT * FROM user WHERE name = ? ORDER BY name DESC, the_tag ASC",
                warpdb.select().from(User.class).where("name = ?", "Bob").orderBy("name").desc().orderBy("tag asc").sql());
    }

    @Test
    public void testSelectForUpdate() {
        assertEquals("SELECT * FROM user FOR UPDATE", warpdb.selectForUpdate().from(User.class).sql());
        assertEquals("SELECT * FROM user ORDER BY name FOR UPDATE", warpdb.selectForUpdate().from(User.class).orderBy("name").sql());
        assertEquals("SELECT email, name FROM user FOR UPDATE", warpdb.selectForUpdate("email", "name").from(User.class).sql());
        assertEquals("SELECT * FROM user LIMIT ?, ? FOR UPDATE", warpdb.selectForUpdate().from(User.class).limit(100).sql());
        assertEquals("SELECT * FROM user WHERE name = ? FOR UPDATE", warpdb.selectForUpdate().from(User.class).where("name = ?", "Bob").sql());
    ...
    public void testAggregate() {
        ...
        assertEquals("SELECT COUNT(*) FROM user WHERE name > ?", warpdb.from(User.class).where("name > ?", "Bob").sql("COUNT(*)"));
        assertEquals("SELECT COUNT(*) FROM user WHERE name > ?", warpdb.select("name", "age").from(User.class).where("name > ?", "Bob").sql("COUNT(*)"));
